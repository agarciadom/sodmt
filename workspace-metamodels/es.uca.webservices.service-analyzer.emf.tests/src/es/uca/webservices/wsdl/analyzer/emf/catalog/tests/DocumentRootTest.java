/**
 */
package es.uca.webservices.wsdl.analyzer.emf.catalog.tests;

import es.uca.webservices.wsdl.analyzer.emf.catalog.DocumentRoot;
import es.uca.webservices.wsdl.analyzer.emf.catalog.MessageCatalogFactory;

import junit.framework.TestCase;

import junit.textui.TestRunner;

/**
 * <!-- begin-user-doc -->
 * A test case for the model object '<em><b>Document Root</b></em>'.
 * <!-- end-user-doc -->
 * <p>
 * The following features are tested:
 * <ul>
 *   <li>{@link es.uca.webservices.wsdl.analyzer.emf.catalog.DocumentRoot#getServices() <em>Services</em>}</li>
 * </ul>
 * </p>
 * @generated
 */
public class DocumentRootTest extends TestCase
{

  /**
   * The fixture for this Document Root test case.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  protected DocumentRoot fixture = null;

  /**
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  public static void main(String[] args)
  {
    TestRunner.run(DocumentRootTest.class);
  }

  /**
   * Constructs a new Document Root test case with the given name.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  public DocumentRootTest(String name)
  {
    super(name);
  }

  /**
   * Sets the fixture for this Document Root test case.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  protected void setFixture(DocumentRoot fixture)
  {
    this.fixture = fixture;
  }

  /**
   * Returns the fixture for this Document Root test case.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  protected DocumentRoot getFixture()
  {
    return fixture;
  }

  /**
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @see junit.framework.TestCase#setUp()
   * @generated
   */
  @Override
  protected void setUp() throws Exception
  {
    setFixture(MessageCatalogFactory.eINSTANCE.createDocumentRoot());
  }

  /**
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @see junit.framework.TestCase#tearDown()
   * @generated
   */
  @Override
  protected void tearDown() throws Exception
  {
    setFixture(null);
  }

  /**
   * Tests the '{@link es.uca.webservices.wsdl.analyzer.emf.catalog.DocumentRoot#getServices() <em>Services</em>}' feature getter.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @see es.uca.webservices.wsdl.analyzer.emf.catalog.DocumentRoot#getServices()
   * @generated
   */
  public void testGetServices()
  {
    // TODO: implement this feature getter test method
    // Ensure that you remove @generated or mark it @generated NOT
    fail();
  }

  /**
   * Tests the '{@link es.uca.webservices.wsdl.analyzer.emf.catalog.DocumentRoot#setServices(es.uca.webservices.wsdl.analyzer.emf.catalog.ServicesType) <em>Services</em>}' feature setter.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @see es.uca.webservices.wsdl.analyzer.emf.catalog.DocumentRoot#setServices(es.uca.webservices.wsdl.analyzer.emf.catalog.ServicesType)
   * @generated
   */
  public void testSetServices()
  {
    // TODO: implement this feature setter test method
    // Ensure that you remove @generated or mark it @generated NOT
    fail();
  }

} //DocumentRootTest
